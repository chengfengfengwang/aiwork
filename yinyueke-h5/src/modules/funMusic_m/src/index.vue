<template>
  <div>
    ...
    <header>
      <img src="../../../assets/img/bookAudioDic/photo_t.png">
      <h2>趣味乐理-视唱练耳（中级）</h2>
      <h2></h2>
    </header>
    <div class="list">
    ...
    $(document).ready(function($) {
      ...
      function buildHtml() {
        ...
        for (var i = 0; i < dicArr.length; i++) {
          var item = dicArr[i];
          var itemText = item.replace('-','. ')
          //var url = baseUrl + encodeURI(name);
          html += `<li><p data-title=${item}>${itemText}</p></li>`;
        }
        $(".list ul").html(html);
      }
      buildHtml();
      ...
      $(".list li").click(function(event) {
        ...
        var title = this.querySelector('p').getAttribute('data-title');
        location.href = `funMusic_m.html?s=${index + 1}&title=${title}#/play`;
        //return;
