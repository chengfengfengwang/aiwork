    ...
    $(document).ready(function($) {
      ...
      function buildHtml() {
        ...
        for (var i = 0; i < dicArr.length; i++) {
          ...
          html +=
            `<li data-title=${name}><p>${(i + 1)}. ${name.replace(/.mp3|.wma/, "")}</p></li>`;
        }
        $(".list ul").html(html);
      }
      buildHtml();
      ...
      $(".list li").click(function(event) {
          ...
          var myTitle = this.getAttribute('data-title')
          location.href = `childMusicBook.html?s=${index+1}&title=${myTitle}`
          return
