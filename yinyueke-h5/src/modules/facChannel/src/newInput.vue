...
export default {
  ...
  created() {
    if (!getQueryVariable("instrument")) {
      ...
      let key = `andelu_${getQueryVariable("instrument")}41`;
      this.countPage(key);
    }
  },
  mounted() {
  ...
  methods: {
    ...
    successCount() {
      this.axios
        .get(`http://58.87.125.111:6363/v1/txsms/tongji?key=vendor_andelu_wx41`)
        .then(res => {});
    },
    btnChange() {
    ...
    getInstruments() {
      this.axios
        .get(`http://58.87.125.111:6363/v1/third/vendor_activity?vendor=andelu&name=andelu41`)
        .then(res => {
          var data = res.data;
          this.instrumentList = [];
          for (let key in data) {
