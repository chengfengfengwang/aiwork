...
<template>
  <div class="page">
    <img src="../../../assets/img/facChannel/weibobg.png" alt class="bg" />
    <div class="bottom">
...
export default {
  ...
  created() {
    if (!getQueryVariable("instrument")) {
      ...
      let key = `weibo_${getQueryVariable("instrument")}`;
      this.countPage(key);
    }
  },
  mounted() {
  ...
  methods: {
    ...
    getInstruments() {
      this.axios
        .get(`http://58.87.125.111:6363/v1/third/vendor_activity?vendor=weibo&name=weibo`)
        .then(res => {
          var data = res.data;
          this.instrumentList = [];
          for (let key in data) {
