...
export default {
  ...
  methods: {
    ...
    downLoad(id){
      window.location.href = `${process.env.XIAOPO}/v2/code_csv/${id}`
      // this.axios
      //   .get(`${process.env.XIAOPO}/v2/code_csv/${id}`)
      //   .then(res => {
      //     this.get();
      //   });
    },
    ...
    get() {
      ...
      if (Object.keys(search).length > 0) {
        var url =
          `${process.env.XIAOPO}/v2/channel_data?page=${this.page}&size=${
            this.pageSize
          }&` + encodeSearchParams(search);
      } else {
        var url = `${process.env.XIAOPO}/v2/channel_data?page=${this.page}&size=${
          this.pageSize
        }`;
      }
console.log(url);
      ...
      //this.axios.get(`${process.env.XIAOPO}/v2/channel_data?type=1&channel=测试`).then(res => {
    ...
    stop(id, status) {
      this.axios 
        .put(`${process.env.XIAOPO}/v2/code/${id}?is_deleted=${status}`)
        .then(res => {
          this.get();
        });
    },
    ...
    handleSubmit() {
      ...
      //return;
      this.axios
        .post(process.env.XIAOPO + "/v2/code", this.formValue)
        .then(res => {
