...
export default {
  ...
  methods: {
    ...
    remove(row) {
      this.axios
        .get(
          `${process.env.JINKANG}/v2/mock/delete_mock/?mock_id=${row.mock_id}`
        )
        .then(res => {
          this.getTestList();
        });
    ...
    getTestList() {
      ...
      if (this.searchBank === 0) {
        var url = `${process.env.JINKANG}/v2/mock/get_many_mock/`;
      } else {
        var url = `${
          process.env.JINKANG
        }/v2/mock/get_many_mock/?questionBank=${this.searchBank}&page=${
          this.page
        }&size=${this.pageSize}`;
      }
