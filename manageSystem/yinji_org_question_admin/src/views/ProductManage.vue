...
export default {
  ...
  methods: {
    ...
    getProductList() {
      ...
      this.axios
        .get(
          `${process.env.JINKANG}/${process.env.VERSION}/goods/get_goods?page=${this.page}&size=${
            this.pageSize
          }`
        )
        .then(res => {
    ...
    removeProduct(id) {
      this.axios
        .get(process.env.JINKANG + "/${process.env.VERSION}/goods/delete_goods?id=" + id)
        .then(res => {
          this.getProductList();
        });
    },
    ...
    handleSubmit(name) {
      ...
      if (this.modalStatus == "create") {
        ...
        //return
        this.axios
          .post(process.env.JINKANG + "/${process.env.VERSION}/goods/update_goods", formdata)
          .then(res => {
