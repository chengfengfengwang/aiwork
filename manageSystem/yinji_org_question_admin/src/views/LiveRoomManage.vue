    ...
    <Modal v-model="modalShow" width="760">
      ...
      <div>
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
          ...
          <FormItem label="course_id">
            <Input v-model.number="formValidate.course_id" placeholder></Input>
          </FormItem>
          <FormItem label="teacher_id">
            <Input v-model.number="formValidate.teacher_id" placeholder></Input>
          </FormItem>
          <FormItem label="lesson_id">
            <Input v-model.number="formValidate.lesson_id" placeholder></Input>
          </FormItem>
...
import { formatTime, getTime, timeToTimeStamp, getDate } from "./../common/util.js";
export default {
  data() {
    return {
      date: new Date(),
          ...
          render: (h, params) => {
            return h("div", [
              h(
                ...
                params.row.record_url?params.row.record_url.slice(0,4) + '...' + params.row.record_url.slice(-4):''
              ),
              h(
                "Button",
                {
  ...
  methods: {
    ...
    handleEditTimeChange(status) {
      if (this.modalStatus == "edit" && status) {
        if (!this.rangeTimeShow) { //只在rangeTimeShow为false时提示
          this.$Message.info({
    ...
    removeRoom(id) {
      this.axios
        .get(`${this.baseUrl}/${process.env.VERSION}/liveroom/delete_room?room_id=${id}`)
        .then(res => {
    ...
    getRoomList(status) {
      ...
      this.axios
        .get(
          `${this.baseUrl}/${process.env.VERSION}/liveroom/all_rooms?page=${this.page}&size=${
            this.pageSize
          }&status=${this.search.status}&keyword=${this.keyWords}`
        )
        .then(res => {
    ...
    handleSubmit(name) {
      ...
      //       return
      //       this.axios
      //         .post(`${this.baseUrl}/${process.env.VERSION}/liveroom/create_room`, formdata,{ headers: {'Content-Type': 'multipart/form-data' }})
      //         .then(res => {
      //           console.log(res);
      //         });
      //     }
      ...
      if (this.modalStatus == "create") {
            var formdata = this.formatForm();
            this.axios
              .post(`${this.baseUrl}/${process.env.VERSION}/liveroom/create_room`, formdata, {
            ...
            var formdata = this.formatForm();
            this.axios
              .post(`${this.baseUrl}/${process.env.VERSION}/liveroom/update_room`, formdata, {
                headers: { "Content-Type": "multipart/form-data" }
              })
              .then(res => {
                console.log(res);
    ...
    formatForm() {
      ...
      newForm.start_time =
        Date.parse(
          getDate(Date.parse(newForm.start_time_date)/1000) + " " + newForm.start_time_time
        ) / 1000;
      newForm.end_time =
        Date.parse(
          getDate(Date.parse(newForm.start_time_date)/1000) + " " + newForm.end_time_time
        ) / 1000;
      [
        "start_time_date",
        "start_time_time",
