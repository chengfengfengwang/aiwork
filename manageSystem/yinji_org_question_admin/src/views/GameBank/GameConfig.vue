...
export default {
  ...
  methods: {
    ...
    searchScsykXuanze() {
      ...
      this.axios
        .get(`${process.env.JINKANG}/${process.env.VERSION}/ranking/get_game/?game_type=25`)
        .then(res => {
    ...
    searchTuozhuai2() {
      this.axios
        .get(
          `${
            process.env.JINKANG
          }/${process.env.VERSION}/question/get_section_line_data/?question_id=${this.tuozhuai2Id}`
        )
        .then(res => {
          this.$Message.success("查询成功");
          this.tuozhuai2Data = res.data;
    ...
    searchQiaoji() {
      this.axios
        .get(
          `${
            process.env.JINKANG
          }/${process.env.VERSION}/question/get_section_line_data/?question_id=${this.qiaojiId}`
        )
        .then(res => {
          this.$Message.success("查询成功");
    ...
    getNameList() {
      this.axios
        .get(`${process.env.XIANSHANG}/${process.env.VERSION}/ranking/game_name/`)
        .then(res => {
          var obj = res.data;
    ...
    getGameList() {
      ...
      this.axios
        .get(
          `${process.env.JINKANG}/${process.env.VERSION}/ranking/get_all_game?page=${
            this.page
          }&size=${this.pageSize}&name=${this.lessonName}&is_test=${
            this.from === "test" ? 1 : 0
          }`
        )
        .then(res => {
    ...
    getProductList() {
      this.tableLoading = true;
      this.axios.get(process.env.JINKANG + "/${process.env.VERSION}/goods/get_goods").then(res => {
        this.tableLoading = false;
        this.gameList = res.data;
      });
    },
    ...
    formateRow(row) {
      ...
      if (this.curGame == "kapai") {
        ...
        if (newRow.material_data && newRow.material_data.length > 0) {
          ...
          this.titaHaveEmptyBeat = newRow.material_data[0].haveEmptyBeat?newRow.material_data[0].haveEmptyBeat:0
        } else {
          this.titaBgt = [];
          this.titaBgMusic = [];
    ...
    removeGame(id) {
      this.axios
        .get(process.env.JINKANG + "/${process.env.VERSION}/ranking/delete_game/?game_id=" + id)
        .then(res => {
          this.getGameList();
        });
    },
    ...
    handleSubmit(name) {
      ...
      this.axios
        .post(
          `${process.env.JINKANG}/${process.env.VERSION}/ranking/create_or_update_game/`,
          formdata
        )
        .then(res => {
    ...
    formatForm() {
      ...
      if (this.modalStatus == "create") {
        ...
        if (this.curGame == "kapai") {
          ...
          newForm.material_data = JSON.stringify(arr);
          //  console.log(this.xuanzeDirection)
