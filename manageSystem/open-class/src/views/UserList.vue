    ...
    <Modal v-model="modalShow" width="760">
      ...
      <div>
        <Form ref="formValidate" :model="formValidate" :label-width="100">
          ...
          <Button v-show="modalStatus=='edit'" style="margin-left:8em" @click="resetPassword">重置密码</Button>
        </Form>
      </div>
      <div slot="footer">
...
export default {
  ...
  methods: {
    resetPassword() {
      //this.curRow;
      this.axios
        .post(`${process.env.JINKANG}/v3/user2/reset_pwd`, {
          username: this.curRow.username,
          password: '123456'
        })
        .then(res => {
    ...
    searchUser(name) {
      if (this.serchValue.trim() === "") {
        ...
        this.axios
          .get(
            `${process.env.JINKANG}/v3/user2/search_user?username=${
              this.serchValue
            }`
          )
          .then(res => {
    ...
    delete(row) {
      this.axios
        .post(`${process.env.JINKANG}/v3/user2/delete_user/`, {
    ...
    edit() {
      this.axios
        .post(
          `${process.env.JINKANG}/v3/user2/update_ascription_permission`,
          this.formValidate
        )
        .then(res => {
    ...
    creat() {
      this.axios
        .post(`${process.env.JINKANG}/v3/user2/add_user`, this.formValidate)
        .then(res => {
