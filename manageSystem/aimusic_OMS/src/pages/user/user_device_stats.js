...
export default class UserDeviceStats extends React.Component {
    ...
    state = {
        ...
        data_columns: [
            ...
            {title: '设备类型名称', dataIndex: '_id', key: '_id',},
    ...
    get_device_count({begin_date_tmp = '', end_date_tmp = ''}) {
        ...
        fetch(this.main_host + '/v3/operating/devices/' + '?begin_date=' + begin_date_tmp + '&end_date='
            + end_date_tmp, {method: 'GET'})
            .then(res => res.json())
            .then(json => {
                    this.setState({items: json.data.ret_query, loading_1: false});
                    if (my_echart) {
                        show_pie(my_echart, json.data, '用户使用设备频次（前20）');
                    }
                }
            )
    }
