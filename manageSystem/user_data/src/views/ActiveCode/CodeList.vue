<template>
  <div>
    ...
    <h1 style="margin:30px;text-align:center">码列表</h1>
    <div>
...
export default {
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: "support_default_amount",
          key: "support_default_amount"
        },
        {
          title: "渠道user_id",
          key: "user_id"
  ...
  mounted() {
    this.batchNumber = JSON.parse(localStorage.getItem("batchNumber"));
    this.getTableData();
  },
  methods: {
      download(){
          location.href = `${process.env.XIAOPO}/v1/code/batch_number/${this.batchNumber.value}?need_export=true&page=${this.page}&size=${this.pageSize}`
      },
    handleUpload(file) {
      this.modalCoverShow = true;
    ...
    getTableData() {
      ...
      this.axios
        .get(
          `${process.env.XIAOPO}/v1/code/batch_number/${this.batchNumber.value}?need_export=false&page=${this.page}&size=${this.pageSize}`
        )
        .then(res => {
