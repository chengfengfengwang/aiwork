<template>
  <div>
    ...
    <Modal draggable v-model="alluserModalShow" width="1160" :styles="{top: '0px'}">
      ...
      <AllUser :fromArr="dataList"  ref="allUser"></AllUser>
      <div slot="footer">
...
export default {
  ...
  methods: {
    deletUsers(){
      console.log(this.curSelectArr)
      if(!this.curSelectArr){
        return
      }else if(this.curSelectArr.length==0){
        return
      }
      //return
      this.axios
    ...
    getGroupUsers() {
      ...
      this.axios.get(url).then(res => {
        ...
        this.total = res.paging?res.paging.total:0;
      });
    },
    delete(id) {
      this.axios
