        ...
        <Sider hide-trigger :style="{background: '#fff',height:'100vh',overflow:'auto'}">
          <Menu :active-name="activeMenuName" theme="light" width="auto" :open-names="openMenuArr">
            <Submenu name="1">
              ...
              <router-link to="/">
                <MenuItem name="ClassList">班级列表</MenuItem>
              </router-link>
              <router-link to="/ClassInfo">
                <MenuItem name="ClassInfo">班级信息</MenuItem>
              </router-link>
            ...
            <Submenu name="User" v-show="$store.getters.permission===0">
              ...
              <router-link to="/User/Permissions">
                <MenuItem name="Permissions">权限管理</MenuItem>
              </router-link>
              <router-link to="/User/Group">
                <MenuItem name="Group">组管理</MenuItem>
              </router-link>
            </Submenu>
            <!-- <router-link to="/Permissions" v-show="$store.getters.permission===0">
              <MenuItem name="Permissions">
                <Icon type="ios-people"/>权限管理
              </MenuItem>
            </router-link> -->
          </Menu>
        </Sider>
...
export default {
  ...
  methods: {
    submitChange() {
      this.axios
        .post(`${process.env.JINKANG}/v3/user/update_pwd`, this.formValidate)
        .then(res => {
          if (res.error === 0) {
