                    ...
                    <Col sm={8}>
                        <Form inline style={{padding: 3}}>
                            ...
                            <FormControl bsSize="sm" placeholder="激活逗号分隔的课程id"
                                         style={{width: '150px', border: 'solid 1px red'}} value={courseIds}
                                         onChange={e => {
                    ...
                    <tbody>
                    {
                        purchases.map((purchase, index) => {
                            return (
                                <tr key={index}>
                                    <td><a
                                        href={`http://aimusic.admin.immusician.com/member/list?t=&do=uid&uid=${purchase.userId}`}
                                        target="_blank">{purchase.user ? `${purchase.user.phone}: ${purchase.userId}: [${purchase.user.device_model}]` : 'NULL'}</a>
                                    </td>
