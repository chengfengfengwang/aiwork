...
function processCourseForUpload(course) {
    ...
    Object.entries(course).forEach(([k, v]) => {
        switch (k) {
            ...
            case 'diplomaExtraConfig':
                if (typeof(v) === 'object'){
                    json[_.snakeCase(k)] = v;
                }else {
                    json[_.snakeCase(k)] = JSON.parse(v);
                }
                break;
        ...
        if (k.toLowerCase().endsWith('price')) {
            json[_.snakeCase(k)] = parseInt(v * 100);
        }
    });
    return json;
}
...
export function getCourse(id) {
    return request('get', '/courses/' + id + '/', {is_admin: 1}).then(json => Promise.resolve(processCourse(json.data)))
}
...
export function getLesson(id) {
    return request('get', '/lessons/' + id, {is_bam: 1}).then(json => Promise.resolve(processLesson(json.data)));
}
...
export function getWechatClassPurchase({page, size, class_id, beginTime, endTime, start_time = null} = {}) {
    return request('get', '/wechat/class_purchase', {
        page,
        size,
        class_id: class_id,
        begin_time: beginTime,
        end_time: endTime,
        day_count_time: start_time
    }).then(json => ({
        ...json,
        data: json.data
    }))
...
export function getWechatClassPaymentData({page, size, class_id, beginTime, endTime} = {}) {
    return request('get', '/wechat/get_class_payment_data', {
        page,
        size,
        class_id: class_id,
        begin_time: beginTime,
...
export function getQuestionPurchases({page, phone} = {}) {
    return request('get', `/question_purchases`, {page, phone}).then(json => ({
        ...json,
        data: json.data.map(keyToCamelCase)
    }))
}

export function activeQuestion(phone, goodsId = "") {
    //激活红框课程
    return request('post', `/active_question`, null, {phone, goods_id: goodsId})
}

export function deactiveQuestion(userId, goodsId) {
    return request('post', `/deactive_question`, null, {user_id: userId, goods_id: goodsId})
}

export function updateQuestionPurchase(id, purchase) {
    return request('patch', `/question_purchases/${id}`, null, keyToSnakeCase(purchase))
}
...
export function updatePromoDescription(id, text) {
    return request('patch', `/promo/promo_description`, {id: id, text: text})
}
...
export function activeCourse(phone, createUser = false, activeAdvance = false, activePartAdvance = false, courseIds = "") {
    if (activeAdvance) {
        ...
        // 激活免费进阶
        return request('post', `/active_course`, null, {
            phone,
            create_user: false,
            active_advance: false,
            active_part_advance: true
        })
    } else {
...
export function activeTestCourse(phone) {
    // 激活新手入门 or 创建用户并激活新手入门
    return request('post', `/active_test_course`, null, {phone})
}
...
export function onGenPromoCode(start, end, channel, phone, goods) {
    return request('post', `/promo/promo_generation`, null, {
        start_id: start,
        end_id: end,
        channel: channel,
        phone: phone,
...
export function getCourseStudies({page, size, phone, courseId, lessonId, userId, instype} = {}) {
    return request('get', '/course_studies', {
        page,
        size,
        phone,
        course_id: courseId,
...
export function getPromoInfo({page, size, keyword} = {}) {
    return request('get', '/promo/promo_info', {page, size, keyword}).then(json => ({
        ...json,
        data: json.data
    }))
}
...
export function getPromoChannel({page, size, channel, beginTime, endTime, start_time = null} = {}) {
    return request('get', '/promo/promo_channel', {
        page,
        size,
        channel,
        begin_time: beginTime,
...
export function onPushMessageGuitar({tagType, content, link, from, title, linktype, instrument_type, course_ids, tels}) {
    return request('post', '/message/message_push_iguitar', {
        tag_type: tagType,
        content: content,
        link: link,
        from: from,
...
export function getScorePlays({page, size, phone, userId, courseId, lessonId}) {
    return request('get', '/score_plays', {
        page,
        size,
        phone,
        user_id: userId,
...
export function login(username, password) {
    return request('post', '/teacher/admin_login', null, {username, password})
...
export function createCoupon(name, describe, coupon_value, send_condition, condition_value, limit_money, start_time,
    ...
    ids = ids.toString();
    console.log(seconds, '---charles----');
    return request('post', `/coupon/create/`, {
        name,
        describe,
        coupon_value,
        send_condition,
...
export function getCoupons(page, size, start_time, instype, money, keyword, times) {
    return request('get', `/coupon/index`, {
        page,
        size,
        end_time: start_time,
        instype,
...
export function getGoods(page, size, instype) {
    return request('get', `/goods/get_goods`, {page, size, instype}).then(json => ({
        ...json,
        data: json.data
    }))
...
export function sendCoupon(tels, coupon_id) {
    return request('post', `/coupon/send`, {tels, coupon_id}).then(json => ({
        ...json,
        data: json.data
    }))
}
...
export function createRecommend(title, state, recommend_type, instrument_type, course_type_one, course_type_two, detail_url, cover_url, item) {
    ...
    return request('post', `/recommend/`, {
        title, state, recommend_type, item,
...
export function updateRecommend(id, title, state, recommend_type, instrument_type, course_type_one,
    ...
    return request('patch', '/recommend/' + id, null, {
        title, state, recommend_type, item,
        instrument_type, course_type_one, course_type_two, detail_url, cover_url
    })
}
...
export function getRecommendDetails({page, size, instrument_type, state, recommend_type, sort}) {
    // 获取推荐详细列表
    return request('get', '/recommend_detail', {
        page,
        size,
        instrument_type,
        state,
...
export function createRecommendDetail(title, state, recommend_type, instrument_type, cover_url, items) {
    ...
    items = item_tmp.join(',');
    return request('post', `/recommend_detail/`, {
        title,
        state,
        recommend_type,
        instrument_type,
...
export function updateRedPoint(id, state, title, instrument_type, course_type_one, course_type_two, items) {
    ...
    return request('patch', '/red_point/' + id, null, {
        title,
        state,
        items,
        instrument_type,
...
export function getActiveTemplates({page, size, state, sort}) {
    return request('get', '/active_template', {page, size, state, sort}).then(json => ({
        ...json, data: json.data.map(processScorePlay)
    }))
}
...
export function createActiveCode(template, count) {
    return request('post', '/active_template/block_create/' + template, {count}).then(json => ({
        ...json, data: json.data
    }))
}
...
export function getActiveCodesStats({}) {
    return request('get', '/active_template/stats/batch', {}).then(json => ({
        ...json, data: json.data.map(processScorePlay)
    }))
}
...
export function togetherMp4(url, piece_v_urls) {
    //激活红框课程
    return request('post', url, null, {piece_v_urls}).then(json => ({
        ...json
    }))
}