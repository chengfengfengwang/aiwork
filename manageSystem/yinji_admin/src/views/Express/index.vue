...
export default {
  ...
  methods: {
    getInfo() {
      this.axios
        .get(`${process.env.PEILUN}/neworder/order_info`)
        .then(res => {});
    },
    dateChange(d) {
    ...
    get() {
      ...
      if (Object.keys(search).length > 0) {
        var url =
          `${process.env.XIAOPO}/${process.env.VERSION}/channel_data?page=${
            this.page
          }&size=${this.pageSize}&` + encodeSearchParams(search);
      } else {
        var url = `${process.env.XIAOPO}/${
          process.env.VERSION
        }/channel_data?page=${this.page}&size=${this.pageSize}`;
      }
      console.log(url);
      //return;
