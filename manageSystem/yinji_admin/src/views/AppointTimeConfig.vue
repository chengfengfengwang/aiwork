...
export default {
  ...
  mounted() {
    ...
    return;
    this.axios
      .post(`${process.env.JINKANG}/${process.env.VERSION}/config/set_live_config`, {
  ...
  methods: {
    ...
    getTimeList() {
      ...
      this.axios.get(`${process.env.JINKANG}/${process.env.VERSION}/config/get_live_config`).then(res => {
    ...
    submit() {
      ...
      //return
      this.axios
        .post(`${process.env.JINKANG}/${process.env.VERSION}/config/set_live_config`, param)
        .then(res => {
          console.log(res);
          this.$Message.success("更新成功啦!");
          this.modalShow = false;
