...
export default {
  ...
  methods: {
    getCourseList(){
      this.axios
        .get(`${process.env.YINGCHUN}/${process.env.VERSION}/new_question/common_courses/`)
        .then(res => {
    ...
    getBankList() {
      ...
      this.axios
        .get(`${process.env.YINGCHUN}/${process.env.VERSION}/new_question/list/`)
        .then(res => {
          this.tableLoading = false;
          this.bankList = res.data;
          //this.total = res.paging.total;
    ...
    handleUpload1(file) {
      ...
      //return false
      this.axios
        .post(`${process.env.YINGCHUN}/${process.env.VERSION}/upload_material/`, formdata, {
          headers: {
    ...
    getProductDetail(id) {
      this.axios.get(process.env.YINGCHUN + "/"+process.env.VERSION+"/goods?id=" + id).then(res => {
        this.tableLoading = false;
        this.getBankList();
      });
    },
    remove(id) {
      this.axios
        .get(process.env.YINGCHUN + "/"+process.env.VERSION+"/new_question/del/?id=" + id)
        .then(res => {
          this.getBankList();
        });
    },
    ...
    handleSubmit(name) {
      if (this.modalStatus == "create") {
        ...
        //return
        this.axios
          .post(process.env.YINGCHUN + "/"+process.env.VERSION+"/new_question/", params)
          .then(res => {
