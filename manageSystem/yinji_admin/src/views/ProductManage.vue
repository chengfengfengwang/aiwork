    ...
    <Modal v-model="modalShow" width="760">
      ...
      <div>
        <Form ref="formValidate" :model="formValidate" :label-width="100">
          ...
          <FormItem label="是否是外链商品">
            <Select v-model="isOutsideGoods" placeholder>
              <Option :value="0">否</Option>
              <Option :value="1">是</Option>
            </Select>
          </FormItem>
          <div v-if="isOutsideGoods===1">
            <FormItem label="外链商品名称">
              <Input v-model="formValidate.outside_goods_name" placeholder="外链商品名称"></Input>
            </FormItem>
...
export default {
  ...
  methods: {
    ...
    getProductList() {
      ...
      this.axios
        .get(
          `${process.env.JINKANG}/${process.env.VERSION}/goods/get_goods?page=${
            this.page
          }&size=${this.pageSize}`
        )
        .then(res => {
    ...
    handleSubmit(name) {
      ...
      if (this.modalStatus == "create") {
        ...
        this.axios
          .post(
            `${process.env.JINKANG}/${process.env.VERSION}/goods/update_goods`,
            formdata
          )
          .then(res => {
    ...
    formatForm() {
      ...
      newForm.outside_goods_old_price = newForm.outside_goods_old_price * 100;
      newForm.outside_goods_price = newForm.outside_goods_price * 100;
