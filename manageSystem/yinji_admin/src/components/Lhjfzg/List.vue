    ...
    <div class="instrument_wrapper">
      <div class="item tool_row" v-for="(item,index) in myArr" :key="item.id">
        ...
        <MyUpload
          ...
          :index="index"
          :idx="index"
          :defaultFile="item.bgMusic"
          v-on:upload-success="bgMusicUploadSuccess"
        />
        <div>
        <span>是否显示节奏：</span>
        <Select v-model="item.showNote" style="width:200px" placeholder>
      ...
      <div>
        <Button @click="add" type="info">新增bgt</Button>
...
export default {
  ...
  methods: {
    ...
    itemUploadSuccess(response, idx, position, file, fileList) {
      this.myArr[idx].bgt = response.data;
    },
    imgUploadSuccess(response, idx, position, file, fileList) {
      this.myArr[idx].img = response.data;
    },
    bgMusicUploadSuccess(response, idx, position, file, fileList) {
      this.myArr[idx].bgMusic = response.data;
    },
    add() {
      this.myArr.push({
        id: Date.now()
