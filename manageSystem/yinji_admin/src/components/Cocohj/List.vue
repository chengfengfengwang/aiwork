  ...
  <div class="upload-group">
    <div class="instrument_wrapper">
      <div class="item tool_row" v-for="(item,index) in myArr" :key="item.id">
        ...
        <div>
          <span style="display:inline-block;vertical-align: top">题干音频：</span>
          <MyUpload
            ...
            :index="index"
            :idx="index"
            :defaultFile="item.question"
            v-on:upload-success="questionUploadSuccess"
          />
        </div>
        <div>
          <span>选项：</span>
          <Item :arr="item.options"/>
        </div>
         <div>
          <span style="display:inline-block;vertical-align: top">答案：</span>
          <Select v-model="item.answer" style="width:200px">
            <Option v-for="n in 5" :key="n" :value="n-1">选项{{n}}</Option>
          </Select>
        </div>
      </div>
      <div>
        <Button @click="add" type="info">新增题目</Button>
